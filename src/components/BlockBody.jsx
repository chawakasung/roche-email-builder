...
export function BlockBody({ kind, props, brand, editing, onEdit, onCloseLetter }) {
  ...
  switch (kind) {
    ...
    case 'notice': return (
      <div className="e-notice" style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', textAlign: 'right' }}>
        <p className="e-notice__note" style={{ textAlign: 'right', alignSelf: 'flex-end' }} {...ce('note')}>{props.note}</p>
        <a className="e-notice__link" style={{ color: brand.primary, alignSelf: 'flex-end' }} href={props.linkHref || '#'} {...ce('linkText')}>{props.linkText}</a>
      </div>
    );
    case 'quote': return (
